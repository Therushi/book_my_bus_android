...
const ManageRoutes: React.FC = () => {
  ...
  const load = useCallback(async () => {
    setRoutes(await RouteRepository.getAll());
  }, []);
  useFocusEffect(
    useCallback(() => {
      load();
  ...
  const handleSave = async () => {
    if (!form.source || !form.destination) {
      Alert.alert('Error', 'Source & destination required');
      return;
    }
    const stopsJson = form.stops
      ? JSON.stringify(
          form.stops
            .split(',')
            .map(s => s.trim())
    ...
    try {
      if (editRoute) {
        await RouteRepository.update(editRoute.id, {
          source: form.source,
  ...
  const handleDelete = (r: Route) => {
    Alert.alert('Delete Route', `Delete ${r.source} → ${r.destination}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          await RouteRepository.delete(r.id);
          load();
        },
      },
    ]);
              ...
              <View style={{ flex: 1 }}>
                ...
                {item.stops && (
                  <Text style={styles.stops}>
                    via {JSON.parse(item.stops).join(', ')}
                  </Text>
                )}
                {item.distance_km ? (
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyDesc}>
              Tap "Add Route" above to create one.
            </Text>
          </View>
        }
      />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modal}>
            <ScrollView>
              ...
              />
              <FormInput
                label="Intermediate Stops (comma separated)"
                value={form.stops}
